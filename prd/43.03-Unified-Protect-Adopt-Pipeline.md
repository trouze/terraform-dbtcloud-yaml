...
State utility expansion delivered on `/removal-management` (now labeled **State Management**):

- Added **State Refresh** controls using Terraform refresh-only workflow:
  - `terraform plan -refresh-only`
  - `terraform apply -refresh-only`
- Added both refresh scopes:
  - all managed resources in state
  - targeted refresh from selected rows (`-target` flags from selected state addresses)
- Added split action layout:
  - **State Refresh** section (sync-from-target behavior)
  - **State Removal** section (existing `terraform state rm` behavior preserved)
- Added plan-viewer parity:
  - plan output can be opened with `View Plan Output`
  - viewer auto-opens after successful refresh plan
- Post-refresh reconcile reload remains aligned with canonical contract (`terraform show -json` parse + UI refresh).
