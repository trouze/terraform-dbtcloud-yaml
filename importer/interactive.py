...
def prompt_credentials() -> dict[str, str | int]:
    ...
    if not current_host:
        host = inquirer.text(
            ...
            validate=lambda result: result.startswith("https://") or "Host URL must start with https://",
...
def prompt_normalize_options() -> dict:
    ...
    """
    console.print("\n[bold cyan]Normalize Command Options[/bold cyan]\n")
    ...
    if recent_files:
        ...
        if selected == "__browse__":
            input_json = inquirer.filepath(
                ...
                only_files=True,
                validate=lambda result: Path(result).exists() and result.endswith(".json") or "File must exist and be a JSON file",
    ...
    mapping_config = inquirer.filepath(
        ...
        only_files=True,
        validate=lambda result: Path(result).exists() or "Mapping config file must exist",
...
def run_normalize_interactive() -> None:
    ...
    console.print("\n[bold cyan]dbt Cloud Importer - Interactive Normalize Mode[/bold cyan]\n")
